...
class Pokemon {
  constructor(PV, Type, Faiblesse, Genre, Entreprise, Cap_Spe, Description, Totem, image)
  {
    this.PV = PV;
    this.Type = Type;
...
let firstPoke = new Pokemon('100', 'Légendaire', 'Feminin', 'BRED', 'Morotisé', 'Augmente sa vitesse à chaque tour', 'salameche.png', 'chat.png');

let tab = [];
tab.push(firstEpisode);
tab.pop();

let numberOfPoke = tab.length;

// ====================================

const body = document.querySelector('body');

document.querySelector('#poke-info').innerHTML = numberOfEpisodes;
...
<div class="box">
  <nav>
    <ul>Vie: ${Pokemon.vie}</ul>
    <ul>Type: ${Pokemon.Type}</ul>
    <ul>Faiblesse: ${Pokemon.Faiblesse}</ul>
    <ul>Genre: ${Pokemon.Genre}</ul>
    <ul>Entreprise: ${Pokemon.Entreprise}</ul>
    <ul>Capacite speciale: ${Pokemon.Cap_Spe}</ul>
    <ul>Description: ${Pokemon.Description}</ul>
    <ul>Totem: ${Pokemon.Totem}</ul>
  </nav>
